...
class Coupon(BaseModel):
    ...
    timer = models.IntegerField(verbose_name="优惠券有效期", default=7,
                                help_text="默认当前优惠券7天有效，如果设置值为-1则表示当前优惠券永久有效")
    condition = models.IntegerField(blank=True, default=0,
                                    verbose_name="满足使用优惠券的价格条件，如果设置值为0,则表示没有任何条件")
...
class UserCoupon(BaseModel):
    ...
    def end_time(self):
        # 优惠券的结束时间
        start_timestamp = self.start_time.timestamp()  # 优惠券的开始时间戳
        use_time = self.coupon.timer * 24 * 60 * 60  # 优惠券的有效期
        end_timestamp = start_timestamp + use_time  # 优惠券的结束时间戳
