...
class User(AbstractUser):
    ...
    credit = models.IntegerField(default=0, verbose_name="用户积分", blank=True)
...
class Credit(BaseModel):
    ...
    def __str__(self):
        return "%s %s %s 贝壳: 在%s" % ( self.user.username, self.OPERA_OPION[self.opera][1], self.number,self.create_time.strftime("%Y-%m-%d %H:%M:%S") )
