...
export default {
  ...
  data() {
    return {
      ...
      playerOptions: {
        ...
        sources: [{ // 播放资源和资源格式
          type: "video/mp4",
          src: "../../static/video/wanglaoshi.mp4" //你的视频地址（必填）
        }],
        poster: "../../static/image/zsl.jpg", //视频封面图
  ...
  methods: {
    get_course_id() {
      let course_id = this.$route.params.courses_id;  // router是跳转页面，route是获取参数
      if (course_id > 0) {
        this.course_id = parseInt(course_id);  // 转换成整数
      } else {
        let self = this;
